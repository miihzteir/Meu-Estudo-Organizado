...
(function (MEO) {
  ...
  V.visaoGeral = function () {
    ...
    let html = `
      ...
      <div class="dash-grid">
        <div class="card dash-block">
          <h3><i data-lucide="circle-check" class="ic"></i> Pendentes para hoje</h3>
        ...
        <div class="card dash-block">
          <h3><i data-lucide="calendar-clock" class="ic"></i> Próximos 7 dias</h3>
    ...
    if (sel) {
      ...
      if (!subjects.length) {
        ...
        subjects.forEach(s => {
          const diaLabel = s.dia !== '' && s.dia != null ? MEO.diasDaSemana[Number(s.dia)].l : '';
          const faltasSubj = db().list('faltas').filter(f => f.subjectId === s.id).reduce((sum, f) => sum + (f.aulas || 1), 0);
          const limiteAulasSubj = s.totalAulas ? Math.floor(s.totalAulas * (s.limiteFaltasPercent != null ? s.limiteFaltasPercent : 25) / 100) : null;
          ...
          html += `<div class="card subject-card" style="border-left-color:${s.cor || 'var(--cor-principal-clara)'}" data-subid="${s.id}">
            ...
            ${diaLabel ? `<span class="meta-row"><i data-lucide="calendar" class="ic-sm"></i> ${diaLabel}${s.horaInicio ? ` · ${s.horaInicio}${s.horaFim ? '–' + s.horaFim : ''}` : ''}</span>` : ''}
            ${(limiteAulasSubj != null || mediaSubj != null) ? `<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:2px;">
              ${limiteAulasSubj != null ? `<span class="pill" style="${faltasSubj >= limiteAulasSubj ? 'background:var(--erro);color:#fff;' : (faltasSubj / limiteAulasSubj >= 0.7 ? 'background:var(--alerta);color:#fff;' : '')}"><i data-lucide="user-x" class="ic-sm"></i> ${faltasSubj}/${limiteAulasSubj} faltas</span>` : ''}
  ...
  V.materiaDetalhe = function (id) {
    ...
    const diaLabel = subj.dia !== '' && subj.dia != null ? MEO.diasDaSemana[Number(subj.dia)].l : 'Dia não definido';
      ...
      <div class="subject-header">
        ...
        <div class="info">
          ...
          <div class="meta">
            ...
            <span><i data-lucide="calendar" class="ic-sm"></i> ${diaLabel}</span>
            ${subj.horaInicio ? `<span><i data-lucide="clock" class="ic-sm"></i> ${subj.horaInicio}${subj.horaFim ? '–' + subj.horaFim : ''}</span>` : ''}
          </div>
        </div>
    ...
    if (!pdfs.length) {
      ...
      pdfs.forEach(p => {
        html += `<div class="item-row" data-pdfid="${p.id}">
          <div class="item-icon"><i data-lucide="file-text" class="ic"></i></div>
          <div class="item-main">
    ...
    if (!items.length) {
      ...
      items.forEach(f => {
        html += `<div class="item-row">
          <div class="item-icon"><i data-lucide="layers-3" class="ic"></i></div>
  ...
  function renderFaltasTab(subj) {
    ...
    const totalAulas = subj.totalAulas || null;
    const limitePercent = subj.limiteFaltasPercent != null ? subj.limiteFaltasPercent : 25;
    const limiteAulas = totalAulas ? Math.floor(totalAulas * limitePercent / 100) : null;
    const pct = limiteAulas ? MEO.clamp(Math.round((totalFaltas / limiteAulas) * 100), 0, 999) : null;
    let corBarra = 'var(--sucesso)', statusTxt = '';
    if (limiteAulas != null) {
      if (totalFaltas >= limiteAulas) { corBarra = 'var(--erro)'; statusTxt = 'Limite de faltas atingido ou ultrapassado.'; }
      else if (pct >= 70) { corBarra = 'var(--alerta)'; statusTxt = `Atenção: ${limiteAulas - totalFaltas} aula${(limiteAulas - totalFaltas) === 1 ? '' : 's'} até o limite.`; }
      else { statusTxt = `${limiteAulas - totalFaltas} aulas de folga até o limite.`; }
    }
    ...
    if (totalAulas) {
      html += `<div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:8px;flex-wrap:wrap;gap:6px;">
        <span style="font-size:14.5px;font-weight:700;">${totalFaltas} de ${limiteAulas} aula${limiteAulas === 1 ? '' : 's'} permitida${limiteAulas === 1 ? '' : 's'} (${limitePercent}% de ${totalAulas})</span>
      ...
      html += `<div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <p style="font-size:13.5px;color:var(--texto-suave);">Você tem <strong style="color:var(--texto);">${totalFaltas}</strong> falta${totalFaltas === 1 ? '' : 's'} registrada${totalFaltas === 1 ? '' : 's'}. Defina o total de aulas previstas em "Editar matéria" para acompanhar o limite automaticamente.</p>
      </div>`;
    }
    html += `</div>`;
    ...
    if (!items.length) {
      ...
      items.forEach(f => {
        html += `<div class="item-row">
          <div class="item-icon"><i data-lucide="user-x" class="ic"></i></div>
          <div class="item-main">
  ...
  function renderNotasTab(subj) {
    ...
    if (!items.length) {
      ...
      items.forEach(g => {
        html += `<div class="item-row">
          <div class="item-icon"><i data-lucide="percent" class="ic"></i></div>
          <div class="item-main">
  ...
  function renderSubjectLinksTab(subj) {
    ...
    if (!items.length) {
      ...
      items.forEach(l => {
        html += `<div class="item-row">
          <div class="item-icon"><i data-lucide="link-2" class="ic"></i></div>
  ...
  function renderObservacoesTab(subj) {
    return `<div class="card">
      ...
      <div style="display:flex;justify-content:flex-end;align-items:center;gap:10px;">
        <span class="saved-indicator" id="obs-saved" style="visibility:hidden;"><i data-lucide="check" class="ic-sm"></i> Salvo</span>
        <button class="btn btn-primary btn-sm" id="btn-salvar-obs"><i data-lucide="check" class="ic-sm"></i> Salvar</button>
      </div>
    </div>`;
  }
  ...
  function wireSubjectTab(root, subj) {
    ...
    if (btnSalvarObs) btnSalvarObs.addEventListener('click', () => {
      ...
      const ind = root.querySelector('#obs-saved');
    ...
    if (obsArea) obsArea.addEventListener('input', MEO.debounce(() => {
      db().upsert('subjects', { id: subj.id, observacoes: obsArea.value });
      const ind = root.querySelector('#obs-saved');
      if (ind) { ind.style.visibility = 'visible'; setTimeout(() => { if (ind) ind.style.visibility = 'hidden'; }, 1600); }
    }, 1200));
  }
  ...
  async function handlePdfUpload(subj, files, root) {
    ...
    for (const file of Array.from(files)) {
      ...
      if (area) area.insertAdjacentHTML('beforeend', `<div class="item-row" id="${rowId}">
        <div class="item-icon"><i data-lucide="file-up" class="ic"></i></div>
  ...
  function wirePomodoro(root) {
    ...
    if ('Notification' in window && Notification.permission === 'default') {
      // não solicita automaticamente aqui; feito em Configurações
    }
  }
  ...
  V.progresso = function () {
    ...
    html += `<div class="stat-grid">
      ${statCard('book-open', subjects.length, 'Matérias')}
      ${statCard('file-text', summaries.length, 'Resumos')}
      ${statCard('layers-3', flashcards.length, 'Flashcards')}
      ${statCard('file', pdfs.length, 'PDFs')}
      ${statCard('circle-check', eventosConcluidos.length, 'Tarefas concluídas')}
      ${statCard('clock', MEO.formatTime(totalMin), 'Minutos estudados')}
    </div>`;
    ...
    const subjectsComFaltasOuNotas = subjects.filter(s => s.totalAulas || db().list('grades').some(g => g.subjectId === s.id));
    if (subjectsComFaltasOuNotas.length) {
      html += `<div class="card" style="margin-top:18px;">
        ...
        ${subjectsComFaltasOuNotas.map(s => {
          ...
          const limiteS = s.totalAulas ? Math.floor(s.totalAulas * (s.limiteFaltasPercent != null ? s.limiteFaltasPercent : 25) / 100) : null;
  ...
  function statCard(icon, num, label) {
    return `<div class="card stat-card"><div class="stat-icon"><i data-lucide="${icon}" class="ic"></i></div><span class="stat-num">${num}</span><span class="stat-label">${label}</span></div>`;
  }
        ...
        <div class="settings-panel">
          ...
          <div class="settings-section ${MEO.state.settingsTab === 'perfil' ? 'active' : ''}" data-section="perfil">
            <div class="card" style="margin-bottom:16px;">
              ...
              <div style="display:flex;gap:10px;flex-wrap:wrap;">
                <button class="btn btn-primary" id="btn-salvar-nome"><i data-lucide="check" class="ic-sm"></i> Salvar</button>
          ...
          <div class="settings-section ${MEO.state.settingsTab === 'graduacao' ? 'active' : ''}" data-section="graduacao">
            <div class="card">
              ...
              <button class="btn btn-primary" id="btn-salvar-grad"><i data-lucide="check" class="ic-sm"></i> Salvar</button>
            </div>
          </div>
          ...
          <div class="settings-section ${MEO.state.settingsTab === 'pomodoro' ? 'active' : ''}" data-section="pomodoro">
            <div class="card">
              ...
              <button class="btn btn-primary" id="btn-salvar-pomo"><i data-lucide="check" class="ic-sm"></i> Salvar</button>
            </div>
          </div>

          <div class="settings-section ${MEO.state.settingsTab === 'notificacoes' ? 'active' : ''}" data-section="notificacoes">
            <div class="card">
              <h3 style="font-size:16px;margin-bottom:10px;">Notificações</h3>
              <p style="color:var(--texto-suave);font-size:13.5px;margin-bottom:14px;">
                As notificações do Pomodoro e da agenda só funcionam enquanto este aplicativo está aberto no seu navegador
                (mesmo em segundo plano). Elas não são enviadas quando o app está totalmente fechado.
              </p>
              <p style="font-size:13px;margin-bottom:14px;">Status atual: <strong>${Notification && Notification.permission === 'granted' ? 'Permitido' : Notification && Notification.permission === 'denied' ? 'Bloqueado pelo navegador' : 'Não solicitado'}</strong></p>
              <button class="btn btn-primary" id="btn-pedir-notif"><i data-lucide="bell" class="ic-sm"></i> Solicitar permissão</button>
            </div>
          </div>
  ...
  function wireConfiguracoes(root) {
    ...
    const notifBtn = root.querySelector('#btn-pedir-notif');
    if (notifBtn) notifBtn.addEventListener('click', async () => {
      if (!('Notification' in window)) { MEO.toast('Notificações não são suportadas neste navegador.', 'error'); return; }
      const perm = await Notification.requestPermission();
      MEO.toast(perm === 'granted' ? 'Notificações permitidas.' : 'Notificações não permitidas.', perm === 'granted' ? 'success' : 'error');
      MEO.rerender();
    });
